...
Note from Dev: so a couple things: for temperature, there are fewer mesic dates than interim and arid, so might want to just use the dates that correspond to the tick cage checks. And humidity needs to be tidied still. 
...
humid_clean <- humidity %>% 
  ...
  mutate(Treatment = case_when(Treatment == "Open"~ "O",
                               Treatment == "T" ~ "T",
                               Treatment == "P" ~"P"))%>%  # replaces treatment names with letters to match temp, this way is fucking stupid but "replace" throws an error I can't figure out
...
microclimate$Date <- mdy(microclimate$Date) # necessary to get dates recognized
